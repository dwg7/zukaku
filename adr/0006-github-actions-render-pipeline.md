...
### 同時実行のrace対策(2026-08-30)

ユーザーからの指摘: 「レンダリング→`docs/responses/`へのcommit書き戻し」という
ステップがある以上、2つのリクエストが近いタイミングでmainにpush/マージされると、
両方のワークフロー実行が同じブランチへの`git push`を競合させ、後勝ちでない方が
失敗する可能性がある。2段構えで対処した:

1. `concurrency: group: atlas-render-${{ github.ref }}` — 同じref(通常はmain)への
   実行を直列化する。PRブランチ(書き戻しをしない)は別グループになるため影響しない。
2. push直前に`git pull --rebase origin <branch>` — 直列化していても、レンダリング中
   (最大で数十秒)に無関係な別コミットがブランチに乗る可能性は残るため、念のため
   pushの直前にrebaseする防御的な一手。

実際にActions上でpush→rebase→pushの経路が壊れていないことを再検証済み。ただし
「2つのリクエストが本当に同時に競合するケース」自体は、意図的に同時多発させて
再現検証してはいない(発生頻度が低く、個人利用規模のツールでは優先度が低いと判断)。
