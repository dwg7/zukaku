...
### さらなる再設計: 上下左右15mm均一+スケールバーも図郭の外へ、回転ロック(2026-08-31)

Print in Browserモードを試したユーザーからのフィードバックを受けて、追加で3点対応した:

1. **マージンを上下左右すべて15mmに統一**(直前の非対称案から変更)。タイトル/
   インデックスは引き続き上マージンの帯に、スケールバーも同様に**下マージンの帯**に
   移動した。ユーザーの言葉を借りると「地図の内容を完全に尊重し、その上に何も
   置かない」という思想(Field Papersを見て感じたこと)を徹底した形。MapLibreの
   `ScaleControl`が生成する`.maplibregl-ctrl-scale`要素そのものを、地図コンテナから
   `#footer`(または`.print-footer`)へ移動(`page.html`)/複製(`docs/index.html`、
   ページごとにMapLibreインスタンスを作り直すため)することで実現した。フォントは
   3mmとかなり控えめにしたが、実機確認では図郭の外に正しく表示されることを確認済み
   (全体スクリーンショットでは小さすぎて見落としやすいので注意)。
2. **方位記号(コンパス)を廃止**。理由は次項の回転ロックとセットで、常に北が上を
   向くならコンパスは冗長という判断。
3. **地図の回転を禁止**(`dragRotate: false`、`pitchWithRotate: false`、
   `touchZoomRotate.disableRotation()`)。これはユーザーが発見した実際のバグの修正でも
   ある: `computePages()`のbbox算出(`[nw.lng, se.lat, se.lng, nw.lat]`という単純な
   min/max合成)は、画面上のNW/SEの2点をunprojectした結果が「本当に地理的な北西・
   南東」であることを前提にしており、これは`bearing`が0のときにしか成り立たない。
   Ctrl+ドラッグで地図を回転させると、この前提が崩れて概要ページの図郭が歪んだ形に
   なる不具合があった。bbox計算を回転対応させる(center+zoom+bearingの3つ組で
   表現し直す等)よりも、**そもそも回転できないようにする**方が単純で確実、という
   ユーザーの判断で対応した。

3点とも`scripts/render/page.html`と`docs/index.html`の両方に同じ数値・同じ構造で
反映し、Playwrightで両モードとも実機確認済み。
