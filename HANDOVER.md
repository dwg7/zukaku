...
- **`rows=1`のような偏ったグリッドで概要ページだけ回転してしまう問題を修正**
  ...
  ようにした。strategy-mixed(非Windows)は無変更。Playwrightで検証済みだが、
  引き続き実機再確認が必要。
...
- **要ユーザー確認: [issue #6](https://github.com/dwg7/zukaku/issues/6)
  (`rows=1`等の偏ったグリッドで概要ページが回転してしまう問題)への対応が
  実際に効いているか**。Playwrightでの検証(Windows偽装UAで回転が発生しなく
  なることを確認)のみで、実際のWindows Edgeでの再印刷確認はまだ。
