...
## D16(ADR 0009追記): 正方形でないグリッドで概要ページのアスペクト比が崩れる不具合を修正

実機報告([dwg7/zukaku#7](https://github.com/dwg7/zukaku/issues/7))により、
行数≠列数のグリッド(例: 2行×3列)で概要ページの地図が縦横比の崩れた状態で
表示されることが判明。原因は、ズームレベルシフト(D6/ADR 0009)のオフスクリーン
ステージが幅を`cols`倍・高さを`rows`倍と軸ごとに別々の係数で拡大しており、
このキャンバスのアスペクト比は`cols===rows`のときしか表示先の固定A4ボックスと
一致しないため。スナップショットを`object-fit: fill`(縦横比無視で箱を埋める)で
はめ込んでいたことで、このズレが地図コンテンツの引き伸ばしとして現れていた。
issue #6の修正とは独立に、ADR 0009導入時点(2026-08-31)から存在した不具合
(たまたま検証グリッドが正方形だったため表面化していなかった)。対応:
`object-fit: contain`に変更し、アスペクト比のズレがあっても地図コンテンツは
